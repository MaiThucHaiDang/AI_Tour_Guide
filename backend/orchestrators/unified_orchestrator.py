...
class UnifiedOrchestrator:
    ...
    async def process_chat_request(
        ...
        if has_database_context:
            ...
            if artifact_info:
                nearby_text = await self._format_nearby_attractions(artifact_info, lang_code)
                if nearby_text:
                    llm_context_parts.append(f"NEARBY_ATTRACTIONS:\n{nearby_text}")
        else:
    ...
    async def _format_nearby_attractions(self, artifact: ArtifactInfo, lang_code: str) -> str:
        """Fetch and format nearby attractions in the same location."""
        try:
            nearby = await get_artifacts_by_location(artifact.loc_id, exclude_art_id=artifact.art_id)
            if not nearby:
                return ""
            lines = []
            for n in nearby:
                name = n.name_vi if lang_code == "vi" else n.name_en
                text = n.history_text_vi if lang_code == "vi" else n.history_text_en
                first_sentence = text.split(". ")[0] if text else ""
                lines.append(f"- {name}: {first_sentence}.")
            return "\n".join(lines[:3])
        except Exception as exc:
            _LOGGER.warning("Failed to fetch nearby attractions: %s", exc)
            return ""
