...
async def get_artifacts_by_location(loc_id: str, exclude_art_id: str | None = None) -> list[ArtifactInfo]:
    """Fetch all artifacts in the same location, optionally excluding the current one."""
    try:
    ...
    async with async_session_factory() as session:
        stmt = select(Artifact).where(Artifact.loc_id == loc_id_int)
        if exclude_art_id:
            try:
                stmt = stmt.where(Artifact.art_id != int(exclude_art_id))
            except (ValueError, TypeError):
                pass
