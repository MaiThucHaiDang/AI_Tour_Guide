...
Moved from: part4/backend/services/voice/conversation_memory.py
Logic preserved exactly — uses centralized config for defaults.
"""
...
class ConversationMemory:
    ...
    def __init__(
        ...
        self._ttl_seconds = ttl_seconds or settings.VOICE_SESSION_TTL_SECONDS
    ...
    def format_history(self, session_id: str) -> str:
        ...
        with self._lock:
            ...
            if history is None:
                return ""
            lines = [
                f"{self._role_label(role)}: {content}"
    ...
    def _cleanup_locked(self) -> None:
        now = time.time()
        stale_ids = [
            sid for sid, h in self._sessions.items()
            if now - h.updated_at > self._ttl_seconds
        ]
